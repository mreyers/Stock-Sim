...
```{r}
start.Date <- as.POSIXct("2009-01-08")
end.Date <- as.POSIXct("2009-12-24")
...
# Now just keep the entries from the year 2000
jan1 <- as.POSIXct("2009-01-01")
dec31 <- as.POSIXct("2009-12-31")
...
# Simulation
  ...
  for(j in 1:nSim){
    globe.j <<- j
    ROR.df[j, i] <- sim_port_return(cleanedYear)
  }
...
more_clean <- temp %>% select(-"2016") %>% melt(id = "sim_n") 
ROR.df.plot <- more_clean

# Plotting the results
ROR.df.plot %>% ggplot(aes(x = value)) + geom_histogram() + facet_wrap(~ variable) +
