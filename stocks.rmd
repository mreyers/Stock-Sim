...
start_open <- sapply(list(sapply(stock_sim, '[[', c(2))), '[[', c(2)) # Broken atm
start_shares <- start_dist / start_open

len_of_year <- dim(stock_sim[[1]])[1]
...
end_close <- sapply(sapply(flipped_list, '[[', c(5)), '[[', c(1))
...
#sim_port_return <- function(cleaned2000){
  ...
  start_value <- 100000
  start_dist <- start_value * port_comp 
start_open <- sapply(sapply(stock_sim, '[[', c(2)), '[[', c(1))
  start_shares <- start_dist / start_open
...
return_in_2000 %>% data.frame() %>% ggplot(aes(x = .)) + geom_histogram()
```
